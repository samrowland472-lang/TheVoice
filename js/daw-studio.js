...
function applyKeysFilter() {
  ...
  midiHeld.forEach((h, pitch) => {
    ...
    const cut = on ? analogCut(keysCutoff, pitch, h.vel) : 12000;
    h.cut = cut;
    try {
...
function fmAmt() {
  return Math.max(0, Math.min(1, Number(keysFm) || 0));
}
...
function startFm(ctx, o1, o2, freq) {
  const gFm = ctx.createGain();
  gFm.gain.value = fmDepthHz(freq);
  if (o2 && o1) {
    o2.connect(gFm);
    gFm.connect(o1.frequency);
  }
  return { gFm };
}

function applyHeldFm(h, t) {
  if (!h || !h.gFm) return;
  try { h.gFm.gain.setTargetAtTime(fmDepthHz(h.freq), t, 0.02); } catch (_) {}
}

function applyKeysFm() {
  const a = audio();
  if (!a) return;
  const t = a.ctx.currentTime;
  midiHeld.forEach((h) => applyHeldFm(h, t));
}

function driftAmt() {
  return Math.max(0, Math.min(1, Number(keysDrift) || 0));
}

function driftDepthHz(freq) {
  if (fx.on.analog === false) return 0;
  return Math.max(0, (freq || 440) * 0.03 * driftAmt());
}

function ensureDriftBuf(ctx) {
  if (driftBuf && driftBuf.sampleRate === ctx.sampleRate) return driftBuf;
  const sr = ctx.sampleRate;
  const n = Math.max(1, Math.floor(sr * 8));
  const buf = ctx.createBuffer(1, n, sr);
  const d = buf.getChannelData(0);
  const a = 1 - Math.exp(-2 * Math.PI * 2 / sr);
  let w = 0;
  let y = 0;
  for (let i = 0; i < n; i++) {
    w += Math.random() * 2 - 1;
...
function startDrift(ctx, oscs, freq, t, stopAt) {
  const gDrift = ctx.createGain();
  gDrift.gain.value = driftDepthHz(freq);
  const src = ctx.createBufferSource();
  const buf = ensureDriftBuf(ctx);
  src.buffer = buf;
  src.loop = true;
  src.playbackRate.value = 0.65 + Math.random() * 0.7;
  src.connect(gDrift);
  (oscs || []).forEach((o) => {
    if (o && o.frequency) gDrift.connect(o.frequency);
  });
  const off = Math.random() * buf.duration;
  try { src.start(t, off); } catch (_) { try { src.start(t); } catch (__) {} }
  if (stopAt != null) {
...
function applyHeldDrift(h, t) {
  if (!h || !h.gDrift) return;
  try { h.gDrift.gain.setTargetAtTime(driftDepthHz(bentHz(h.freq)), t, 0.02); } catch (_) {}
}

function applyKeysDrift() {
  const a = audio();
  if (!a) return;
  const t = a.ctx.currentTime;
  midiHeld.forEach((h) => applyHeldDrift(h, t));
}
...
function uniMix() {
  return Math.max(0, Math.min(1, keysUni)) * 0.45;
}

function uniRatios() {
  const c = 4 + Math.max(0, Math.min(1, keysUni)) * 18;
...
function vibDepthHz(freq) {
  if (fx.on.analog === false) return 0;
  const amt = Math.max(0, Math.min(1, keysVib));
...
function lfoDelay() {
  return Math.max(0, Math.min(2, Number(keysLfoDelay) || 0));
}
  ...
  const arm = (g, depth) => {
    ...
    if (delay > 0.008 || fade > 0.008) {
      ...
      if (fade > 0.008) {
        ...
        g.gain.linearRampToValueAtTime(depth, t + delay + fade);
      } else {
        g.gain.setValueAtTime(depth, t + delay);
      }
    } else {
      g.gain.value = depth;
    }
...
function paintDevices() {
  ...
  root.innerHTML = `
    <article class="abl-dev${fx.on.analog === false ? ' bypassed' : ''}" data-dev="analog">
      ...
      <div class="abl-dev-body">
        ...
        ${knob('abl-fm', 'FM', 0, 1, 0.01, keysFm)}
        ${knob('abl-drf', 'Drf', 0, 1, 0.01, keysDrift)}
  ...
  const fm = root.querySelector('#abl-fm');
  const drf = root.querySelector('#abl-drf');
  ...
  bindAnalog(fm, 'fm', 'FM', (v) => { keysFm = v; applyKeysFm(); });
  bindAnalog(drf, 'drift', 'Drf', (v) => { keysDrift = v; applyKeysDrift(); });
...
function applyMidiTarget(target, vel) {
  ...
  } else if (target.type === 'fm') {
    keysFm = vel;
    applyKeysFm();
    const el = document.getElementById('abl-fm');
    if (el) el.value = String(keysFm);
  } else if (target.type === 'drift') {
    keysDrift = vel;
    applyKeysDrift();
    const el = document.getElementById('abl-drf');
    if (el) el.value = String(keysDrift);
  ...
  } else if (target.type === 'lfod') {
    keysLfoDelay = vel * 2;
    const el = document.getElementById('abl-ldly');
    if (el) el.value = String(keysLfoDelay);
  } else if (target.type === 'lfof') {
    keysLfoFade = vel * 2;
