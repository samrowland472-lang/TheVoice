...
function lastOuterXToFirstHole(from: Element, targetAxis: "x" | "y"): boolean {
  const key = pointKey(from);
  if (!key || !/^path-\d+$/.test(key)) return false;
  const axis = from.getAttribute?.("data-path-axis");
  if (axis && axis !== "x") return false;
...
export function shouldTabFromLastOuterXToFirstHoleY(
  from: Element | null | undefined,
  shift: boolean,
): boolean {
  if (shift || !from || !(from instanceof Element)) return false;
  if (lastOuterXToFirstHole(from, "x")) return false;
  return lastOuterXToFirstHole(from, "y") || lastOuterXToFirstHoleFallback(from);
}

function lastOuterXToFirstHoleFallback(from: Element): boolean {
  const key = pointKey(from);
  if (!key || !/^path-\d+$/.test(key)) return false;
  const axis = from.getAttribute?.("data-path-axis");
  if (axis && axis !== "x") return false;
