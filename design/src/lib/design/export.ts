...
function nodeSvg(n: DesignNode): string {
  ...
  if (n.kind === "path") {
    ...
    const d = pathD(p.x, p.y, p.points, p.closed || cut.length > 0) + cut.map((h) => pathD(p.x, p.y, h, true)).join("");
    const rule = cut.length ? "evenodd" : pathFillRule(p);
    const outer = `<path d="${esc(d)}" fill="${esc(fill)}" fill-rule="${rule}" clip-rule="${rule}"${stroke}${opacity}${rot}${shadowAttr(n)}/>`;
    const islandMarkup = islands
      .filter((ring) => ring.length >= 3)
      .map((ring) => {
        const id = pathD(p.x, p.y, ring, true);
        return `<path d="${esc(id)}" fill="${esc(fill)}" fill-rule="nonzero"${opacity}${rot}${shadowAttr(n)}/>`;
      })
      .join("");
    return outer + islandMarkup;
  }
  if (n.kind === "ellipse") {
