        ...
        {showPeek && (
          <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex flex-col items-center">
            ...
            <div
              ...
              onPointerDown={(e) => {
                e.stopPropagation();
                peekScrubbing.current = true;
                e.currentTarget.setPointerCapture(e.pointerId);
                const rect = e.currentTarget.getBoundingClientRect();
                const idx = peekScrubIndex(e.clientX, rect.left, rect.width, pages.length);
            ...
            >
              {pages.map((p, n) => (
                <button
                  ...
                  onMouseEnter={() => {
                    ...
                    if (shiftHeld && n !== i) setPeekNamedId(p.id);
                  }}
                  onMouseLeave={() => {
                    setPeekIndexHover(false);
                    setPeekNamedId((cur) => (cur === p.id ? null : cur));
                  }}
                  onFocus={() => {
                    if (n === i) setPeekIndexHover(true);
                    if (shiftHeld && n !== i) setPeekNamedId(p.id);
              ...
              {(peekIndexHover || shiftHeld) && (
                <span className="ml-1 flex min-w-0 max-w-[min(40vw,14rem)] items-baseline gap-1 text-[10px] font-medium tracking-wide text-ink-faint/80">
                  ...
                  {shiftHeld && (pages.find((p) => p.id === peekNamedId) ?? pages[i + 1]) && (
                    <span className="min-w-0 truncate [mask-image:linear-gradient(90deg,#000_70%,transparent)]">
                      {(pages.find((p) => p.id === peekNamedId) ?? pages[i + 1])!.name}
                    </span>
                  )}
                </span>
              )}
