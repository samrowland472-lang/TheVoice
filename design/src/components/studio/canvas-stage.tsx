...
export function CanvasStage() {
  ...
  function onPointerDown(e: React.PointerEvent) {
    ...
    e.preventDefault();
    e.currentTarget.setPointerCapture(e.pointerId);
    const hit = s.doc ? hitTop(s.doc.nodes, d.x, d.y) : null;
    if (hit) {
      if (!s.selection.includes(hit.id)) s.select([hit.id], e.shiftKey);
      else if (e.shiftKey) s.select([hit.id], true);
      const live = useDesign.getState();
      live.commit();
      const sel = live.selection;
      const nodes = live.doc?.nodes ?? [];
      moveRef.current = {
        kind: "move",
        sx: d.x,
        sy: d.y,
        orig: nodes.filter((n) => sel.includes(n.id)).map((n) => ({ id: n.id, x: n.x, y: n.y })),
      };
      guidesRef.current = { x: [], y: [], spaces: [] };
      return;
    }
    if (!e.shiftKey) s.select([]);
    marqueeRef.current = { sx: d.x, sy: d.y, x: d.x, y: d.y, w: 0, h: 0, additive: e.shiftKey };
    setHoverTick((n) => n + 1);
  }
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    if (moving && s.doc) {
      let dx = d.x - moving.sx;
      let dy = d.y - moving.sy;
      const proposed = moving.orig.map((o) => ({ id: o.id, x: o.x + dx, y: o.y + dy }));
      const liveNodes = proposed
        .map((p) => {
          const n = s.doc!.nodes.find((x) => x.id === p.id);
          return n ? { ...n, x: p.x, y: p.y } : null;
        })
        .filter((n): n is NonNullable<typeof n> => Boolean(n));
      const others = s.doc.nodes.filter((n) => !moving.orig.some((o) => o.id === n.id) && n.visible);
      if (s.snap && !e.altKey) {
        const extra = {
          x: (s.doc.guides ?? []).filter((g) => g.axis === "x").map((g) => g.pos),
          y: (s.doc.guides ?? []).filter((g) => g.axis === "y").map((g) => g.pos),
        };
        const snapped = smartSnap(liveNodes, others, s.doc.artboard, 6, extra);
        dx += snapped.dx;
        dy += snapped.dy;
  ...
  function onPointerUp(e: React.PointerEvent) {
    ...
    if (mq) {
      const s = useDesign.getState();
      const thresh = 4 / Math.max(s.viewport.zoom, 0.01);
      if (s.doc && (mq.w > thresh || mq.h > thresh)) {
        const ids = nodesInMarquee(s.doc.nodes, { x: mq.x, y: mq.y, w: mq.w, h: mq.h }, "intersect");
        if (mq.additive) {
          const cur = new Set(s.selection as string[]);
