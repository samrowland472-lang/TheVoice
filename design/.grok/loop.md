...
1. Path inspector Shift+Tab from first hole header onto last outer-path x when last outer y is missing and hole 0 has no point fields.

## Done

- Path inspector Shift+Tab from first hole header lands on last outer-path y when hole 0 has no point fields. `shouldShiftTabFromFirstHoleHeaderToLastOuterY` + `pickLastOuterLastPointYTabTarget`; hole 0 x/y still preferred when present. Holds Points and Holes list scroll.
...
2026-09-11 07:28 BST — Shift+Tab from first hole header to last outer-path y when hole 0 has no point fields.

## Next recommended

Path inspector Shift+Tab from first hole header onto last outer-path x when last outer y is missing and hole 0 has no point fields.
