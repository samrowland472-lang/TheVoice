...
test("second document pointer-leave after muted keep-clear Shift-held document pointer-move path stays muted so a leave cannot flash the next-frame name", () => {
  assert.match(src, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerLeave/);
  assert.match(src, /return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerMove\(opts\)/);
  assert.match(chrome, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerLeave/);
  assert.match(chrome, /applyQuietKeepClearDocPointerLeave/);
  assert.match(chrome, /onDocPointerLeave/);
  assert.match(chrome, /document.addEventListener\("pointerleave"/);
  assert.match(barrel, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerLeave/);

  const late = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerLeave({
    muted: true,
    namedId: "stale-frame",
  ...
  const named = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerLeave({
    muted: true,
    namedId: null,
    fallbackId: "frame-next",
