...
function peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerOver(opts) {
  return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerEnter(opts);
}

test("second document pointer-over after muted keep-clear Shift-held document pointer-enter path stays muted so an over cannot flash the next-frame name", () => {
  assert.match(src, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerOver/);
  assert.match(src, /return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerEnter\(opts\)/);
  assert.match(chrome, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerOver/);
  assert.match(chrome, /applyQuietKeepClearDocPointerOver/);
  assert.match(chrome, /onDocPointerOver/);
  assert.match(chrome, /document.addEventListener\("pointerover"/);
  assert.match(barrel, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerOver/);

  const late = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerOver({
    muted: true,
    namedId: "stale-frame",
  ...
  const named = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocPointerOver({
    muted: true,
    namedId: null,
    fallbackId: "frame-next",
