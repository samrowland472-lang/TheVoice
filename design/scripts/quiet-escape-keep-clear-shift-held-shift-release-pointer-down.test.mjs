...
function peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove(opts) {
...
test("document pointer-down after muted keep-clear Shift-held path stays muted so a down cannot flash the next-frame name", () => {
  assert.match(src, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerDown/);
  assert.match(src, /return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove\(opts\)/);
  assert.match(chrome, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerDown/);
  assert.match(chrome, /applyQuietKeepClearPointerDown/);
  assert.match(chrome, /onPointerDown/);
  assert.match(chrome, /document.addEventListener\("pointerdown"/);
  assert.match(barrel, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerDown/);

  const late = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerDown({
    muted: true,
    namedId: "stale-frame",
  ...
  const named = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerDown({
    muted: true,
    namedId: null,
    fallbackId: "frame-next",
