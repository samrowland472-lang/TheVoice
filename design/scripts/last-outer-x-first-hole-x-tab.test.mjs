...
test("Tab from last outer-path x hops to first hole-path x when first hole y is missing", () => {
  assert.match(a, /function shouldTabFromLastOuterXToFirstHoleX/);
  ...
  assert.match(barrel, /shouldTabFromLastOuterXToFirstHoleX/);
  assert.match(barrel, /pickFirstHoleFirstPointXTabTarget/);
  assert.match(view, /function focusFirstHoleFirstX/);
  assert.match(view, /shouldTabFromLastOuterXToFirstHoleX/);
  ...
  assert.match(xBlock, /focusFirstHoleFirstX/);
  assert.match(ui, /function focusFirstHoleFirstX/);
  assert.match(ui, /shouldTabFromLastOuterXToFirstHoleX/);
});

test("last outer x to first hole x holds Points and Holes list scroll after growth", () => {
