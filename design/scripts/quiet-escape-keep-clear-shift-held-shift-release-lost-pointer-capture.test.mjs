...
test("lost pointer capture mid muted keep-clear Shift-held path stays muted so a stolen pointer cannot flash the next-frame name", () => {
  assert.match(src, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseLostPointerCapture/);
  assert.match(src, /return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerCancel\(opts\)/);
  assert.match(chrome, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseLostPointerCapture/);
  assert.match(chrome, /applyQuietKeepClearLostPointerCapture/);
  assert.match(chrome, /lostpointercapture/);
  assert.match(barrel, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseLostPointerCapture/);

  const stolen = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseLostPointerCapture({
    muted: true,
    namedId: "stale-frame",
    fallbackId: "frame-next",
  });
  assert.equal(stolen.muted, true);
  assert.equal(stolen.namedId, null);
  assert.equal(stolen.showCaption, false);
  assert.equal(stolen.captionId, null);
  assert.equal(stolen.mutedPointerUpKeep, false);

  const named = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseLostPointerCapture({
    muted: true,
    namedId: null,
    fallbackId: "frame-next",
