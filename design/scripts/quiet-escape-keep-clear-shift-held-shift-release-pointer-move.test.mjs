...
test("document pointer-move after muted keep-clear Shift-held path stays muted so a move cannot flash the next-frame name", () => {
  assert.match(src, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove/);
  assert.match(src, /return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerOver\(opts\)/);
  assert.match(chrome, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove/);
  assert.match(chrome, /applyQuietKeepClearPointerMove/);
  assert.match(chrome, /onPointerMove/);
  assert.match(chrome, /document.addEventListener\("pointermove"/);
  assert.match(barrel, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove/);

  const late = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove({
    muted: true,
    namedId: "stale-frame",
  ...
  const named = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleasePointerMove({
    muted: true,
    namedId: null,
    fallbackId: "frame-next",
