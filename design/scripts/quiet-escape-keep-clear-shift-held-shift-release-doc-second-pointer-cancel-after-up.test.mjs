...
function peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocSecondPointerCancel(opts) {
  return peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocSecondPointerUp(opts);
}

test("second document pointer-cancel after muted keep-clear Shift-held second pointer-up path stays routed through the second pointer-up mute helper", () => {
  assert.match(chrome, /applyQuietKeepClearDocSecondPointerCancel/);
  assert.match(chrome, /applyQuietKeepClearDocSecondPointerUp/);
  assert.match(chrome, /onDocPointerCancel/);
  assert.match(
    chrome,
    /const applyQuietKeepClearDocSecondPointerCancel = \(\) => \{\s*applyQuietKeepClearDocSecondPointerUp\(\);/,
  );
  assert.match(chrome, /document.addEventListener\("pointercancel", onDocPointerCancel\)/);
  assert.match(src, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocSecondPointerCancel/);
  assert.match(barrel, /peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocSecondPointerCancel/);

  const late = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocSecondPointerCancel({
    muted: true,
    namedId: "stale-frame",
  ...
  const named = peekCaptionAfterQuietEscapeAfterKeepClearShiftHeldShiftReleaseDocSecondPointerCancel({
    muted: true,
    namedId: null,
    fallbackId: "frame-next",
