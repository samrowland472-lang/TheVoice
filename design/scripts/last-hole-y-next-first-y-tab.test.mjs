...
test("Tab from last hole-path y hops to next hole first-point y", () => {
  assert.match(a, /function shouldTabFromLastHoleYToNextFirstY/);
  assert.match(a, /axisWanted: "x" \| "y", targetAxis: "x" \| "y"|targetAxis: "x" \| "y"/);
  ...
  assert.match(view, /shouldTabFromLastHoleYToNextFirstY/);
...
test("last-hole y to next first y holds Points and Holes list scroll after growth", () => {
